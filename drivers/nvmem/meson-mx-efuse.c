...
static int meson_mx_efuse_read_secure_firmware(void *context,
					       ...
					       size_t bytes)
{
	...
	for (i = 0; i < bytes; i += efuse->config.word_size) {
		ret = meson_secure_firmware_efuse_read(offset + i, sizeof(tmp),
						       &tmp);
		if (ret)
			return ret;
...
static int meson_mx_efuse_probe(struct platform_device *pdev)
{
	...
	if (meson_secure_firmware_available())
		efuse->config.reg_read = meson_mx_efuse_read_secure_firmware;
	else
