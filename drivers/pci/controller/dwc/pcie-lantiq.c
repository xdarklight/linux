...
static int lantiq_pcie_hw_init(struct dw_pcie *pci)
{
	...
	ret = clk_prepare_enable(ltq_pcie->ctrl_clk);
	if (ret)
...
err_disable_ctrl_clk:
	clk_disable_unprepare(ltq_pcie->ctrl_clk);
err_phy_power_off:
...
static int lantiq_pcie_host_init(struct pcie_port *pp)
{
	...
	for (i = 0; i < 5; i++) {
		...
		/* try again due to the workaround described above */
...
static int lantiq_pcie_probe(struct platform_device *pdev)
{
	...
	ltq_pcie->ctrl_clk = devm_clk_get(dev, "ctrl");
	if (IS_ERR(ltq_pcie->ctrl_clk))
		return PTR_ERR(ltq_pcie->ctrl_clk);
