...
#define  GSWIP_PCE_PCTRL_1_LRNLIM	GENMASK(7, 0) /* Learning limit: 0x0 = disabled, 0xff = unlimited */
			       ...
			       dsa_fdb_dump_cb_t *cb, void *data)
{
	...
	for (i = 0; i < 2048; i++) {
		...
		if (is_static)
			matches_port = !!(mac_bridge.val[0] & BIT(port));
		else
			matches_port = port == FIELD_GET(GSWIP_TABLE_MAC_BRIDGE_PORT,
							 mac_bridge.val[0]);
		...
		dev_err(priv->dev, "%s(%d): %s FDB entry with port(mask) 0x%02x, FID %u and VLAN ID %u: %pM\n", __func__, port, is_static ? "static" : "learned", mac_bridge.val[0], fid, vid, addr);
