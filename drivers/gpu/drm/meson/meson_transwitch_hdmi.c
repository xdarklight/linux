...
static int meson_txc_hdmi_hw_init(struct meson_txc_hdmi *priv,
				  struct device *dev)
{
	const unsigned long ddc_i2c_bus_clk_hz = 500 * 1000;
	const unsigned long sys_clk_hz = 24 * 1000 * 1000;
	int ret;
	...
	regmap_write(priv->regmap, TX_HDCP_CONFIG3,
		     FIELD_PREP(TX_HDCP_CONFIG3_DDC_I2C_BUS_CLOCK_TIME_DIVIDER,
				DIV_ROUND_UP_ULL(sys_clk_hz, ddc_i2c_bus_clk_hz) - 1));
...
static int meson_txc_hdmi_probe(struct platform_device *pdev)
{
	...
	priv->next_bridge = devm_drm_of_get_bridge(dev, dev->of_node, 1, 0);
	if (IS_ERR(priv->next_bridge))
		return dev_err_probe(dev, PTR_ERR(priv->next_bridge),
				     "Failed to get next bridge from port@1\n");
