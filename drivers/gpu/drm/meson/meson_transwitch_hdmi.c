...
static const struct regmap_config meson_txc_hdmi_bus_regmap_config = {
	.name = "bus",
	.reg_bits = 16,
	.val_bits = 16,
	.reg_stride = 1,
...
static int meson_txc_hdmi_child_regmap_read(void *context, unsigned int addr,
					    unsigned int *data)
{
...
static int meson_txc_hdmi_child_regmap_write(void *context, unsigned int addr,
					     unsigned int data)
{
	struct regmap *bus_regmap = context;
...
static const struct regmap_config meson_txc_hdmi_bridge_regmap_config = {
	.name = "bridge",
	.reg_bits = 16,
	.val_bits = 8,
	.reg_stride = 1,
	.reg_read = meson_txc_hdmi_child_regmap_read,
	.reg_write = meson_txc_hdmi_child_regmap_write,
	.max_register = 0x7ff,
	.fast_io = true,
};

static const struct regmap_range meson_txc_hdmi_other_regmap_ranges[] = {
	regmap_reg_range(0x8000, 0x800c),
};

static const struct regmap_access_table meson_txc_hdmi_other_regmap_access = {
	.yes_ranges = meson_txc_hdmi_other_regmap_ranges,
	.n_yes_ranges = ARRAY_SIZE(meson_txc_hdmi_other_regmap_ranges),
};

static const struct regmap_config meson_txc_hdmi_other_regmap_config = {
	.name = "other",
	.reg_bits = 16,
	.val_bits = 16,
	.reg_stride = 1,
	.reg_read = meson_txc_hdmi_child_regmap_read,
	.reg_write = meson_txc_hdmi_child_regmap_write,
	.rd_table = &meson_txc_hdmi_other_regmap_access,
	.wr_table = &meson_txc_hdmi_other_regmap_access,
	.max_register = 0xffff,
	.fast_io = true,
};
					     ...
					     unsigned int tx_pkt_reg)
{
	...
	meson_txc_hdmi_write_infoframe(priv->tx_regmap, tx_pkt_reg, buf,
					     ...
					     const struct drm_display_mode *mode)
{
	...
	drm_hdmi_avi_infoframe_quant_range(&frame, conn, mode,
	...
	meson_txc_hdmi_write_infoframe(priv->tx_regmap,
...
static void meson_txc_hdmi_update_codec_status(struct meson_txc_hdmi *priv,
...
static void meson_txc_hdmi_bridge_enable(struct drm_bridge *bridge)
{
	...
	meson_txc_hdmi_config_serializer_clock(priv);
	meson_txc_hdmi_reconfig_packet_setting(priv);
...
static void meson_txc_hdmi_bridge_disable(struct drm_bridge *bridge)
{
	struct meson_txc_hdmi *priv = bridge_to_meson_txc_hdmi(bridge);
...
static void meson_txc_hdmi_bridge_mode_set(struct drm_bridge *bridge,
					   const struct drm_display_mode *mode,
					   const struct drm_display_mode *adj)
{
	struct drm_connector *conn = drm_panel_bridge_connector(bridge);
	struct meson_txc_hdmi *priv = bridge_to_meson_txc_hdmi(bridge);
	...
	if (priv->sink_is_hdmi) {
		...
		meson_txc_hdmi_set_avi_infoframe(priv, conn, mode);
		meson_txc_hdmi_set_vendor_infoframe(priv, conn, mode);
...
static enum drm_connector_status meson_txc_hdmi_bridge_detect(struct drm_bridge *bridge)
{
	...
	meson_txc_hdmi_update_codec_status(priv, status);
...
static const struct drm_bridge_funcs meson_txc_hdmi_bridge_funcs = {
	...
	.enable = meson_txc_hdmi_bridge_enable,
	.disable = meson_txc_hdmi_bridge_disable,
	.mode_set = meson_txc_hdmi_bridge_mode_set,
	.mode_valid = meson_txc_hdmi_bridge_mode_valid,
	.detect = meson_txc_hdmi_bridge_detect,
	.get_edid = meson_txc_hdmi_bridge_get_edid,
};

static irqreturn_t meson_txc_hdmi_irq_thread(int irq, void *dev_id)
{
	enum drm_connector_status status;
	struct meson_txc_hdmi *priv = dev_id;
...
static irqreturn_t meson_txc_hdmi_irq_handler(int irq, void *dev_id)
{
	struct meson_txc_hdmi *priv = dev_id;
	irqreturn_t ret;

	regmap_read(priv->other_regmap, HDMI_OTHER_INTR_STAT,
		    &priv->intr_stat);
	if (!priv->intr_stat)
		return IRQ_NONE;
...
static int meson_txc_hdmi_hw_init(struct meson_txc_hdmi *priv)
{
	...
	ret = devm_request_threaded_irq(priv->dev, priv->irq,
					meson_txc_hdmi_irq_handler,
					meson_txc_hdmi_irq_thread, 0, NULL, priv);
	if (ret) {
					  ...
					  struct hdmi_codec_params *hparms)
{
	...
	for (i = 0; i < ARRAY_SIZE(hparms->iec.status); i++) {
		...
		regmap_write(priv->tx_regmap, TX_IEC60958_SUB1_OFFSET + i,
			     sub1);
		regmap_write(priv->tx_regmap, TX_IEC60958_SUB2_OFFSET + i,
			     sub2);
	}
...
static int meson_txc_hdmi_hdmi_codec_audio_startup(struct device *dev, void *data)
{
	struct meson_txc_hdmi *priv = data;
...
static void meson_txc_hdmi_hdmi_codec_audio_shutdown(struct device *dev, void *data)
{
	struct meson_txc_hdmi *priv = data;
...
static int meson_txc_hdmi_hdmi_codec_mute_stream(struct device *dev, void *data,
...
{
	struct meson_txc_hdmi *priv = data;
	struct drm_connector *conn = drm_panel_bridge_connector(&priv->bridge);

	memcpy(buf, conn->eld, min_t(size_t, MAX_ELD_BYTES, len));
...
static int meson_txc_hdmi_hdmi_codec_hook_plugged_cb(struct device *dev, void *data,
						...
						struct device *codec_dev)
{
	...
	meson_txc_hdmi_update_codec_status(priv, conn->status);
	mutex_unlock(&priv->codec_mutex);
			       ...
			       void *data)
{
	...
	bus_regmap = devm_regmap_init(dev, NULL, base,
				      &meson_txc_hdmi_bus_regmap_config);
	if (IS_ERR(bus_regmap))
		return PTR_ERR(bus_regmap);

	priv->tx_regmap = devm_regmap_init(dev, NULL, bus_regmap,
					   &meson_txc_hdmi_bridge_regmap_config);
	if (IS_ERR(priv->tx_regmap))
		return PTR_ERR(priv->tx_regmap);

	priv->other_regmap = devm_regmap_init(dev, NULL, bus_regmap,
					      &meson_txc_hdmi_other_regmap_config);
	if (IS_ERR(priv->other_regmap))
		return PTR_ERR(priv->other_regmap);
	...
	if (ret) {
		dev_err_probe(priv->dev, ret, "Failed to enable the pclk\n");
		return ret;
	}
