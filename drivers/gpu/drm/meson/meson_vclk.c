					 ...
					 unsigned int rate_khz)
{
	...
	if (priv->vid_clk_needs_exclusive_put[clk_id])
		clk_rate_exclusive_put(clk);

	ret = clk_set_rate_exclusive(clk, rate_khz * 1000UL);

	priv->vid_clk_needs_exclusive_put[clk_id] = !ret;

	return ret;
}

static int meson_vclk_setup_ccf(struct meson_drm *priv, unsigned int target,
