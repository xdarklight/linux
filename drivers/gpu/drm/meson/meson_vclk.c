				 ...
				 unsigned long long venc_freq)
{
	...
	if (ret) {
		dev_err(priv->dev, "Failed to set TMDS clock to %lluHz: %d\n",
			phy_freq, ret);
...
out_enable_clocks:
	...
	if (ret)
		dev_err(priv->dev,
			"Failed to re-enable the VENC clock at %lluHz: %d\n",
			venc_freq, ret);
	else
	...
	if (ret)
		dev_err(priv->dev,
			"Failed to re-enable the pixel clock at %lluHz: %d\n",
			dac_freq, ret);
	else
		      ...
		      bool hdmi_use_enci)
{
	...
	if (meson_vpu_is_compatible(priv, VPU_COMPATIBLE_M8) ||
		...
		dev_err(priv->dev, "%s(target: %u, phy: %llu, dac: %llu, venc: %llu, hdmi_use_enci: %u)\n", __func__, target, phy_freq, dac_freq, venc_freq, hdmi_use_enci);
