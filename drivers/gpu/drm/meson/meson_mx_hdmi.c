...
static void meson_mx_hdmi_bridge_enable(struct drm_bridge *bridge)
{
	...
	regmap_update_bits(hdmi->other_regmap, HDMI_OTHER_CTRL1,
...
static int meson_mx_hdmi_register(struct meson_mx_hdmi *hdmi)
{
	...
	hdmi->anx = anx_hdmi_14_tx_probe(hdmi->dev);
	if (IS_ERR(hdmi->anx))
		return PTR_ERR(hdmi->anx);
...
static irqreturn_t meson_mx_hdmi_irq_handler(int irq, void *dev_id)
{
	...
	regmap_read(hdmi->other_regmap, HDMI_OTHER_INTR_STAT, &regval);
	if (!regval)
		return IRQ_NONE;
	...
	regmap_write(hdmi->other_regmap, HDMI_OTHER_INTR_STAT_CLR, regval);
...
{
	if (hdmi->anx)
		anx_hdmi_14_tx_remove(hdmi->anx);
...
static int meson_mx_hdmi_init(struct meson_mx_hdmi *hdmi, void __iomem *base)
{
	...
	regmap_write(hdmi->other_regmap, HDMI_OTHER_INTR_STAT_CLR,
	...
	regmap_write(hdmi->other_regmap, HDMI_OTHER_INTR_MASKN,
			      ...
			      void *data)
{
	...
	struct regmap *bus_regmap, *bridge_regmap;
	...
	bridge_regmap = devm_regmap_init(dev, NULL, bus_regmap,
					 &meson_mx_hdmi_bridge_regmap_config);
	if (IS_ERR(bridge_regmap))
		return PTR_ERR(bridge_regmap);

	hdmi->other_regmap = devm_regmap_init(dev, NULL, bus_regmap,
					&meson_mx_hdmi_other_regmap_config);
	if (IS_ERR(hdmi->other_regmap))
		return PTR_ERR(hdmi->other_regmap);
	...
	ret = meson_mx_hdmi_register(hdmi);
	if (ret) {
