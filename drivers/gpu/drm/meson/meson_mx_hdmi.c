...
static void meson_mx_hdmi_unmask_irqs(struct meson_mx_hdmi *hdmi)
{
	regmap_write(hdmi->regmap, HDMI_OTHER_INTR_MASKN, 0x0);
}

static void meson_mx_hdmi_mask_irqs(struct meson_mx_hdmi *hdmi)
{
...
static void meson_mx_hdmi_hw_enable(struct meson_mx_hdmi *hdmi)
{
	...
	regmap_write(hdmi->regmap, TX_HDCP_CONFIG0,
		     TX_HDCP_CONFIG0_ROM_ENCRYPT_OFF);
	...
	meson_mx_hdmi_unmask_irqs(hdmi);
}
