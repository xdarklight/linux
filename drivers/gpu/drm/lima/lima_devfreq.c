...
static const char * const lima_devfreq_regulator_names[] = { "mali" };
...
int lima_devfreq_init(struct lima_device *ldev)
{
	...
	ldev->devfreq.opp_table = opp_table;

	opp_table = dev_pm_opp_set_regulators(dev,
					      lima_devfreq_regulator_names, 1);
	if (IS_ERR(opp_table)) {
		ret = PTR_ERR(opp_table);
		goto err_opp_put_clkname;
	}
...
err_opp_put_regulators:
	dev_pm_opp_put_regulators(ldev->devfreq.opp_table);
err_opp_put_clkname:
