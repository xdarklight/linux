...
#define MESON_P_NAND_CMD					0x00
	...
	#define MESON_P_NAND_CMD_ECC_MODE_BCH24			0x3
	#define MESON_P_NAND_CMD_ECC_MODE_BCH30			0x4
	#define MESON_P_NAND_CMD_ECC_MODE_BCH40			0x5
	#define MESON_P_NAND_CMD_ECC_MODE_BCH50			0x6
	#define MESON_P_NAND_CMD_ECC_MODE_BCH60			0x7
	/* unset to write: */
...
static void meson_nand_select_chip(struct mtd_info *mtd, int ce)
{
	...
	if (ce >= MESON_NAND_MAX_CHIP_SELECTS) {
		nfc->selected_chip = -1;
		return;
	}
...
static u8 meson_nand_get_ecc_mode(struct mtd_info *mtd)
{
	...
	case 24:
		return MESON_P_NAND_CMD_ECC_MODE_BCH24;
	case 30:
		return MESON_P_NAND_CMD_ECC_MODE_BCH30;
	case 40:
		return MESON_P_NAND_CMD_ECC_MODE_BCH40;
	case 50:
		return MESON_P_NAND_CMD_ECC_MODE_BCH50;
	case 60:
		return MESON_P_NAND_CMD_ECC_MODE_BCH60;
	default:
		WARN_ON(1);
		return MESON_P_NAND_CMD_ECC_MODE_NONE;
	}
...
static int meson_nand_dma_wait_read_complete(struct mtd_info *mtd)
{
	struct nand_chip *chip = mtd_to_nand(mtd);
	struct meson_nfc *nfc = nand_get_controller_data(chip);
...
printk("%s: %s page %d\n", __func__, direction == DMA_FROM_DEVICE ? "read" : "write", page);
	...
	/* configure the seed (for "ran mode") based on the page: */
	...
	else if (chip->ecc.mode == NAND_ECC_NONE)
		cmd |= MESON_P_NAND_CMD_DMA_RAN;

	if (chip->ecc.mode != NAND_ECC_NONE && chip->ecc_step_ds == SZ_512) {
	...
	dma_unmap_single(nfc->dev, data_dma_addr, nfc->data_dma_size,
			 direction);

	if (direction == DMA_FROM_DEVICE)
		return meson_nand_dma_wait_read_complete(mtd);
	else
		return 0;
}
				...
				uint8_t *buf, int oob_required, int page)
{
	...
	meson_nand_dma_xfer_page(mtd, page, DMA_FROM_DEVICE);

	memcpy(buf, nfc->data_dma_buf, mtd->writesize);
				 ...
				 int page)
{
	...
	memcpy(nfc->data_dma_buf, buf, mtd->writesize);

	meson_nand_dma_xfer_page(mtd, page, DMA_TO_DEVICE);
			       ...
			       int page)
{
	...
	ret = meson_nand_dma_xfer_page(mtd, page, DMA_FROM_DEVICE);
	if (ret)
		return ret;
...
static int meson_nand_correct(struct mtd_info *mtd, uint8_t *dat,
			      uint8_t *read_ecc, uint8_t *calc_ecc)
{
	struct nand_chip *chip = mtd_to_nand(mtd);
	struct meson_nfc *nfc = nand_get_controller_data(chip);
	int i;
	u32 info_data, info_index, error_count, bitflips = 0;

	if (mtd->writesize % chip->ecc_step_ds)
		return -EINVAL;

	for (i = 0; i < (mtd->writesize / chip->ecc_step_ds); i++) {
		info_index = (i * BITS_PER_BYTE / sizeof(int));
		if (meson_nand_is_new_oob_mode(mtd))
			info_index += 4;

		info_data = nfc->info_dma_buf[info_index];

		error_count = FIELD_GET(MESON_P_NAND_INFO_PAGE_ERR_CNT_MASK,
					info_data);
		...
		bitflips = max(bitflips, error_count);
	}
...
static int meson_nand_apply_timings(struct mtd_info *mtd,
...
static int meson_nand_dma_init(struct mtd_info *mtd)
{
	...
	nfc->data_dma_size = mtd->writesize + mtd->oobsize;
	nfc->data_dma_buf = devm_kzalloc(nfc->dev, nfc->data_dma_size,
					 GFP_KERNEL);
	if (!nfc->data_dma_buf)
		return -ENOMEM;
