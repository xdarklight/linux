				   ...
				   const struct nand_interface_config *conf)
{
	...
	if (nfc->caps->regs[NFC_NFI_DEBUG_CON1] >= 0) {
		u32 tsel = 0;
		...
		temp = nfi_readl(nfc, nfc->caps->regs[NFC_NFI_DEBUG_CON1]);
		temp &= ~STROBE_MASK;
		temp |= tsel << STROBE_SHIFT;
		nfi_writel(nfc, temp, nfc->caps->regs[NFC_NFI_DEBUG_CON1]);
	}
				    ...
				    u32 sectors)
{
	...
	if (nfc->caps->has_dma_support) {
		...
		mtk_ecc_check_and_correct(nfc->ecc, &stats, chip->ecc.size,
					  buf, oob_ptr(chip, start), start);
	}
				    ...
				    u32 start, u32 spare, u8 *buf, int raw)
{
	...
	for (i = 0; i < min_t(u32, sectors, chip->ecc.steps); i++) {
		u8 *sector_buf = data_ptr(chip, buf, i);

		mtk_nfc_read_buf(chip, sector_buf, nand->ecc.size);

		if (raw) {
			mtk_nfc_read_buf(chip, oob_ptr(chip, i),
					 fdm->reg_size);
			mtk_nfc_read_buf(chip,
					 oob_ptr(chip, chip->ecc.steps + i),
					 spare - fdm->reg_size);
		} else {
			rc = mtk_ecc_wait_sector_done(nfc->ecc, ECC_DECODE, i);
			...
			bitflips += mtk_nfc_update_ecc_stats(mtd, sector_buf,
							     start + i, 1);
		}
	}
				 ...
				 int page)
{
	...
	if (!nfc->caps->has_dma_support) {
		ret = mtk_nfc_read_subpage(mtd, chip, 0, mtd->writesize,
					   buf ? buf : nfc->buffer, page, 1);
		if (ret < 0)
			return ret;
...
static const struct mtk_nfc_caps mtk_nfc_caps_mt2701 = {
	...
	.regs = {
		...
		[NFC_NFI_DEBUG_CON1] = NFI_DEBUG_CON1,
	},
	.has_pad_clk = true,
