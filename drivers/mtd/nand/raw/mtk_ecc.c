...
#define		DEC_EL_BYTE_POS_SHIFT	(3)
#define		DEC_EL_BIT_POS_MASK	GENMASK(3, 0)
...
static u32 mtc_ecc_get_num_error_bits(struct mtk_ecc *ecc, u32 offset,
				      u8 sector)
{
	u32 err, width;

	width = ALIGN(fls(ecc->caps->err_mask), 4);
		       ...
		       int sectors)
{
	...
	for (i = 0; i < sectors; i++) {
		offset = (i >> 2) << 2;
		err = mtc_ecc_get_num_error_bits(ecc, offset, 0);
...
void mtk_ecc_check_and_correct(struct mtk_ecc *ecc,
			       struct mtk_ecc_stats *stats, int ecc_size,
			       u8 *sector_buf, u8 *fdm_buf, u8 sector)
{
	u32 i, num_error_bits, fdm_end_bits;
	u32 bitflips = 0;

	stats->corrected = 0;
	stats->failed = 0;

	num_error_bits = mtc_ecc_get_num_error_bits(ecc, 0, sector);
	...
	for (i = 0; i < num_error_bits; i++) {
		u32 error_byte_pos, error_bit_loc, error_bit_mask;
		u32 error_locations, sector_error_location;

		error_locations = readl(ecc->regs + ECC_DECEL0 + (i / 2));
		...
		error_byte_pos = sector_error_location >> DEC_EL_BYTE_POS_SHIFT;
		error_bit_loc = FIELD_GET(DEC_EL_BIT_POS_MASK,
					  sector_error_location);
		error_bit_mask = 1 << error_bit_loc;

		if (error_bit_loc < (ecc_size << DEC_EL_BYTE_POS_SHIFT)) {
			sector_buf[error_byte_pos] ^= error_bit_mask;
		} else if (error_bit_loc < fdm_end_bits) {
			fdm_buf[error_byte_pos - ecc_size] ^= error_bit_mask;
		}

		bitflips++;
	}

	stats->corrected += num_error_bits;
	stats->bitflips = bitflips;
}
EXPORT_SYMBOL(mtk_ecc_check_and_correct);
