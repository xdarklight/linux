...
#define MESON_AO_SEC_SD_CFG12					0xf0
	...
	#define MESON_AO_SEC_SD_CFG12_VREF_EN			BIT(15)
					...
					const struct iio_chan_spec *chan)
{
	...
	/*
	 * Meson8m2/8b/8 and older SoCs don't have the MESON_SAR_ADC_REG11
	 * register, so any changes to them are no-op below.
	 */
	if (chan->type == IIO_TEMP) {
		...
		regmap_update_bits(priv->regmap, MESON_SAR_ADC_DELTA_10,
				   MESON_SAR_ADC_DELTA_10_TS_VBG_EN, 0);
		regmap_update_bits(priv->regmap, MESON_SAR_ADC_REG11,
					     ...
					     unsigned int *ao_sec_sd_cfg12)
{
	...
	/* The vendor kernel uses a hard-coded value THERMAL_VREF is enabled */
	if (priv->thermal_vref ||
	    *ao_sec_sd_cfg12 & MESON_AO_SEC_SD_CFG12_THERMAL_VREF)
...
static int meson_sar_adc_temp_sensor_init_gxl(struct iio_dev *indio_dev)
{
	...
	if (priv->thermal_vref ||
	    ao_sec_sd_cfg12 & MESON_AO_SEC_SD_CFG12_VREF_EN)
		priv->temperature_divider = 153;
	else
		priv->temperature_divider = 171;
...
static const struct meson_sar_adc_param meson_sar_adc_gxl_param = {
	...
	.resolution = 12,
	.temperature_multiplier = 10,
	.temperature_divider = 0, /* 153 or 171, detected at runtime */
