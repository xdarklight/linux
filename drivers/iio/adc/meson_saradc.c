...
static unsigned int meson_saradc_get_fifo_count(struct iio_dev *indio_dev)
{
	...
	return FIELD_GET(SAR_ADC_REG0_FIFO_COUNT_MASK, reg);
}

static int meson_saradc_wait_busy_clear(struct iio_dev *indio_dev)
{
	struct meson_saradc_priv *priv = iio_priv(indio_dev);
	int reg, timeout = 1000;

	do {
		udelay(10);
		regmap_read(priv->regmap, SAR_ADC_REG0, &reg);
	} while (FIELD_GET(SAR_ADC_REG0_BUSY_MASK, reg) && timeout--);
...
{
	struct meson_saradc_priv *priv = iio_priv(indio_dev);
	int ret, reg, fifo_chan, fifo_val, sum = 0, count = 0;
...
#if 0 // FIXME
	for (ret = 0; ret <= SAR_ADC_REG13; ret += 0x4) {
		regmap_read(priv->regmap, ret, &reg);
		dev_warn(&indio_dev->dev, "0x%02x = 0x%08x\n", ret, reg);
	}
#endif
					...
					const struct iio_chan_spec *chan)
{
	 ...
	 */
	val = FIELD_PREP(SAR_ADC_CHAN_LIST_MAX_INDEX_MASK, 0);
	regmap_update_bits(priv->regmap, SAR_ADC_CHAN_LIST,
			   SAR_ADC_CHAN_LIST_MAX_INDEX_MASK, val);

	/* this is the mapping between channel index (in our case always 0)
	 * and the actual input channel.
	 */
	FIELD_PREP(SAR_ADC_CHAN_CHAN_ENTRY_MASK(0), chan->channel);
	regmap_update_bits(priv->regmap, SAR_ADC_CHAN_LIST,
			   SAR_ADC_CHAN_CHAN_ENTRY_MASK(0), val);

	val = FIELD_PREP(SAR_ADC_DETECT_IDLE_SW_DETECT_MODE_MUX_MASK,
			 chan->channel);
	regmap_update_bits(priv->regmap, SAR_ADC_DETECT_IDLE_SW,
			   SAR_ADC_DETECT_IDLE_SW_DETECT_MODE_MUX_MASK, val);

#if 0
	// TODO: fixes GXBB?
	regmap_update_bits(priv->regmap, SAR_ADC_DETECT_IDLE_SW,
			   SAR_ADC_DETECT_IDLE_SW_DETECT_MODE_YM_DRIVE_SW, 0);
#endif

	val = FIELD_PREP(SAR_ADC_DETECT_IDLE_SW_IDLE_MODE_MUX_SEL_MASK,
			 chan->channel);
	regmap_update_bits(priv->regmap, SAR_ADC_DETECT_IDLE_SW,
			   SAR_ADC_DETECT_IDLE_SW_IDLE_MODE_MUX_SEL_MASK, val);
}
					  ...
					  enum meson_saradc_ch7_mux_mode mode)
{
	...
	val = FIELD_PREP(SAR_ADC_REG3_CTRL_CHAN7_MUX_SEL_MASK, mode);
	regmap_update_bits(priv->regmap, SAR_ADC_REG3,
			   SAR_ADC_REG3_CTRL_CHAN7_MUX_SEL_MASK, val);

#if 0
	/* TODO: fixes GXBB? */
	regmap_update_bits(priv->regmap, SAR_ADC_REG3,
			   SAR_ADC_REG3_CTRL_CONT_RING_COUNTER_EN, 0);
#endif
...
static int meson_saradc_init(struct iio_dev *indio_dev)
{
	...
	// FIXME: test
	regmap_update_bits(priv->regmap, SAR_ADC_REG0,
			   SAR_ADC_REG0_FIFO_CNT_IRQ_MASK,
			   FIELD_PREP(SAR_ADC_REG0_FIFO_CNT_IRQ_MASK, 8));

	regmap_update_bits(priv->regmap, SAR_ADC_REG0,
			   SAR_ADC_REG0_FIFO_IRQ_EN,
			   SAR_ADC_REG0_FIFO_IRQ_EN);
#endif

	regmap_read(priv->regmap, SAR_ADC_REG3, &val);

	priv->bl30_managed = !!(val & SAR_ADC_REG3_BL30_INITIALIZED);
