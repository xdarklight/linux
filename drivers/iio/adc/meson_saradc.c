...
static char meson_sar_adc_get_fifo_channel(const struct iio_chan_spec *chan)
{
					 ...
					 int *val)
{
	...
	if (fifo_chan != meson_sar_adc_get_fifo_channel(chan)) {
		dev_err(&indio_dev->dev,
			"ADC FIFO entry belongs to channel %d instead of %d\n",
			fifo_chan, meson_sar_adc_get_fifo_channel(chan));
		return -EINVAL;
	}
...
{
	struct meson_sar_adc_priv *priv = iio_priv(indio_dev);
	int val, channel = meson_sar_adc_get_fifo_channel(chan);
...
{
	struct meson_sar_adc_priv *priv = iio_priv(indio_dev);
	int channel = meson_sar_adc_get_fifo_channel(chan);
	u32 regval;
				    ...
				    int *val)
{
	...
	if (ret) {
		dev_warn(indio_dev->dev.parent,
			 ...
			 meson_sar_adc_get_fifo_channel(chan), ret);
		return ret;
	}
