...
#define MESON_HHI_DPLL_TOP_0_TSC_BIT5				BIT(9)
...
struct meson_sar_adc_priv {
	...
	int					calibbias;
	int					calibscale;
	struct nvmem_cell			*temperature_calibration_cell;
	struct nvmem_cell			*cvbs_trimming_cell;
...
static char meson_sar_adc_get_channel(const struct iio_chan_spec *chan)
{
	if (chan->type == IIO_TEMP)
		return MESON_SAR_ADC_VOLTAGE_AND_TEMP_CHANNEL;
					 ...
					 int *val)
{
	...
	if (fifo_chan != meson_sar_adc_get_channel(chan)) {
		dev_err(&indio_dev->dev,
			"ADC FIFO entry belongs to channel %d instead of %d\n",
			fifo_chan, meson_sar_adc_get_channel(chan));
		return -EINVAL;
	}
...
{
	struct meson_sar_adc_priv *priv = iio_priv(indio_dev);
	int val, channel = meson_sar_adc_get_channel(chan);
...
{
	struct meson_sar_adc_priv *priv = iio_priv(indio_dev);
	int channel = meson_sar_adc_get_channel(chan);
	u32 regval;
				    ...
				    int *val)
{
	...
	if (ret) {
		dev_warn(indio_dev->dev.parent,
			 ...
			 meson_sar_adc_get_channel(chan), ret);
		return ret;
	}
					   ...
					   int *val, int *val2, long mask)
{
	...
	case IIO_CHAN_INFO_SCALE:
		if (chan->type == IIO_VOLTAGE) {
			...
			*val = priv->data->param->temperature_multiplier;
			*val2 = priv->data->param->temperature_divider;
	...
	case IIO_CHAN_INFO_OFFSET:
		*val = DIV_ROUND_CLOSEST(MESON_SAR_ADC_TEMP_OFFSET *
					 priv->data->param->temperature_divider,
					 priv->data->param->temperature_multiplier);
...
static int meson_sar_adc_temp_sensor_init(struct iio_dev *indio_dev)
{
	struct meson_sar_adc_priv *priv = iio_priv(indio_dev);
	size_t read_len;
	u8 *buf, trimming_bits, calibration_flag;
	int ret;

	trimming_bits = priv->data->param->temperature_trimming_bits;

	priv->tsc_regmap = syscon_regmap_lookup_by_phandle(
						indio_dev->dev.parent->of_node,
						"amlogic,hhi-sysctrl");
	if (IS_ERR(priv->tsc_regmap)) {
		ret = PTR_ERR(priv->tsc_regmap);
		if (ret == -ENODEV && trimming_bits == 4)
			priv->tsc_regmap = NULL;
		else
			return 0;
	}

	priv->temperature_calibration_cell = devm_nvmem_cell_get(
						&indio_dev->dev,
						"temperature_calibration");
	if (IS_ERR(priv->temperature_calibration_cell)) {
		ret = PTR_ERR(priv->temperature_calibration_cell);
		if (ret == -EPROBE_DEFER)
			return -EPROBE_DEFER;
		else
			return 0;
	...
	priv->cvbs_trimming_cell = devm_nvmem_cell_get(&indio_dev->dev,
						       "cvbs_trimming");
	if (IS_ERR(priv->cvbs_trimming_cell)) {
		ret = PTR_ERR(priv->cvbs_trimming_cell);
		if (ret == -EPROBE_DEFER)
			return -EPROBE_DEFER;
		else
			return 0;
	...
	buf = nvmem_cell_read(priv->cvbs_trimming_cell, &read_len);
	if (IS_ERR(buf)) {
		dev_err(indio_dev->dev.parent,
			"failed to read cvb_trimming cell\n");
		return PTR_ERR(buf);
	} else if (read_len != MESON_SAR_ADC_EFUSE_BYTES) {
		dev_err(indio_dev->dev.parent,
			"invalid read size of cvb_trimming cell\n");
		return -EINVAL;
	}

	calibration_flag = buf[1] >> 4;

	kfree(buf);

	read_len = MESON_SAR_ADC_EFUSE_BYTES;
	buf = nvmem_cell_read(priv->temperature_calibration_cell, &read_len);
	if (IS_ERR(buf)) {
		dev_err(indio_dev->dev.parent,
			"failed to read temperature_calibration cell\n");
		return PTR_ERR(buf);
	} else if (read_len != MESON_SAR_ADC_EFUSE_BYTES) {
		dev_err(indio_dev->dev.parent,
			"invalid read size of temperature_calibration cell\n");
		return -EINVAL;
	}

	if (calibration_flag == MESON_SAR_ADC_TEMP_EFUSE_MUST_RIGHT ||
	    calibration_flag == MESON_SAR_ADC_TEMP_EFUSE_FIXED)
		priv->temperature_sensor_calibrated = buf[3] & BIT(7);
	else
		priv->temperature_sensor_calibrated = false;

	priv->temperature_sensor_adc_val = buf[2];
	priv->temperature_sensor_adc_val |= (buf[3] & ~BIT(7)) << 8;
	priv->temperature_sensor_adc_val >>= trimming_bits;

	priv->temperature_sensor_coefficient = buf[2];
	priv->temperature_sensor_coefficient &= BIT(trimming_bits) - 1;
...
static int meson_sar_adc_init(struct iio_dev *indio_dev)
{
	...
	if (priv->temperature_sensor_calibrated) {
		...
		/*
		 * set the TSC (temperature sensor coefficient) to make the
		 * temperature reading work.
		 */
		...
		/* the 5th bit of the TSC is located in another register */
		if (priv->data->param->temperature_trimming_bits == 5) {
			if (priv->temperature_sensor_coefficient & BIT(5))
				regval = MESON_HHI_DPLL_TOP_0_TSC_BIT5;
			else
				regval = 0;

			regmap_update_bits(priv->tsc_regmap,
					   MESON_HHI_DPLL_TOP_0,
					   MESON_HHI_DPLL_TOP_0_TSC_BIT5,
					   regval);
		}
	} else {
		regmap_update_bits(priv->regmap, MESON_SAR_ADC_DELTA_10,
...
static int meson_sar_adc_probe(struct platform_device *pdev)
{
	...
	indio_dev->channels = priv->data->param->channels;
	indio_dev->num_channels = priv->data->param->num_channels;
	...
	if (priv->data->param->temperature_trimming_bits) {
