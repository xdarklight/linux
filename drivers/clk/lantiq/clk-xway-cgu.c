...
u32 ltq_xway_cgu_r32(u32 reg)
{
	return __raw_readl(ltq_cgu_membase + reg);
}

void ltq_xway_cgu_w32(u32 val, u32 reg)
{
	__raw_writel(val, ltq_cgu_membase + reg);
}

static void ltq_xway_cgu_gpio_clk_set(struct clk_hw *hw, bool enable)
{
	struct ltq_cgu_gpio_clk *gpio_clk = to_ltq_cgu_gpio_clk(hw);
	unsigned int val, enable_bits;
...
{
	ltq_xway_cgu_gpio_clk_set(hw, true);
...
{
	ltq_xway_cgu_gpio_clk_set(hw, false);
}
