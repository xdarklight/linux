...
/* The standard clk_ops for the PMU gates on the xway platform. */
...
#define PMU_GATE(_id, _name, _func, _parent, _flags, _dev, _con)	\
	{								\
		.id		= _id,					\
		.name		= _name,				\
		.function	= _func,				\
 ...
 * @ifccr The CGU interface clock control register offset
 */
