...
#define sys1_w32(x, y)		__raw_writel((x), sys1_membase + (y))
#define sys1_r32(x)		__raw_readl(sys1_membase + (x))
#define sys1_w32_mask(clear, set, reg)	\
		sys1_w32((sys1_r32(reg) & ~(clear)) | (set), reg)

#define status_r32(x)		__raw_readl(status_membase + (x))

static void __iomem *sys1_membase, *syseth_membase, *status_membase;
...
static void falcon_gpe_enable(void)
{
	...
	/* apply new frequency */
	sys1_w32_mask(7 << (GPPC_OFFSET + 1),
			freq << (GPPC_OFFSET + 2), SYS1_INFRAC);
	udelay(1);

	/* enable new frequency */
	sys1_w32_mask(0, 1 << (GPPC_OFFSET + 1), SYS1_INFRAC);
	udelay(1);
}
...
static void falcon_register_system_clks(void)
{
	...
	if (sys1_r32(SYS1_CPU0CC) & CPU0CC_CPUDIV)
		cpu_rate = CLOCK_200M;
	else
		cpu_rate = CLOCK_400M;
...
static struct ltq_falcon_sys_gate falcon_sys1_gates[] __initdata = {
	...
	{SYS1_GATE_GPTC, 15, "gptc", "fpi", "1e100e00.gptc"}, // TODO: is bit 15 correct?
...
static void __init falcon_sys1_clk_init_dt(struct device_node *np)
{
	...
	sys1_membase = ioremap_nocache(res_sys1.start,
	...
	falcon_sys_gates_init(np, &sys1_data, sys1_membase, falcon_sys1_gates,
...
static void __init falcon_syseth_clk_init_dt(struct device_node *np)
{
	...
	falcon_sys_gates_init(np, &syseth_data, syseth_membase, falcon_syseth_gates,
