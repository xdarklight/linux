...
#define LTQ_ICU_NUM_IM			5
#define LTQ_ICU_NUM_IR_PER_IM		32
#define LTQ_ICU_NUM_IRQS		(LTQ_ICU_NUM_IM * LTQ_ICU_NUM_IR_PER_IM)

#define LTQ_ICU_HWIRQ_TO_BITMASK(hwirq)	BIT((hwirq) % LTQ_ICU_NUM_IR_PER_IM)

/* the performance counter */
#define LTQ_ICU_IM4_PERF_IRQ		((4 * LTQ_ICU_NUM_IR_PER_IM) + 31)
...
static void ltq_icu_irq_handler(struct irq_desc *desc)
{
	...
	irq = ioread32be(priv->base[vpe] + LTQ_ICU_IOSR);
	if (irq == 0)
		return;
	...
	/*
	 * silicon bug causes only the msb set to 1 to be valid.
	 * all other bits might be bogus
	 */
	irq = __fls(irq);
	hwirq = irq + (LTQ_ICU_NUM_IR_PER_IM * priv->im);

	generic_handle_irq(irq_find_mapping(priv->domain, hwirq));
...
{
	struct irq_domain *parent_domain, *icu_domain;
	struct ltq_icu_priv *priv;
	unsigned int im, vpe;
	void __iomem *base;
	int irq;

	parent_domain = irq_find_host(of_irq_find_parent(node));
	if (!parent_domain) {
		pr_err("%pOF: No interrupt-parent found\n", node);
		return -EINVAL;
	}

	priv = kcalloc(LTQ_ICU_NUM_IM, sizeof(*priv), GFP_KERNEL);
	if (!priv)
		return -ENOMEM;
	...
	for_each_possible_cpu(vpe) {
		...
		for (im = 0; im < LTQ_ICU_NUM_IM; im++) {
			...
			iowrite32be(U32_MAX, priv[im].base[vpe] + LTQ_ICU_ISR);
			iowrite32be(U32_MAX, priv[im].base[vpe] + LTQ_ICU_IMR);
	...
	icu_domain = irq_domain_add_hierarchy(parent_domain, 0,
					      LTQ_ICU_NUM_IRQS, node,
					      &ltq_icu_irq_domain_ops, priv);
	if (IS_ERR(icu_domain)) {
		pr_err("%pOF: Failed to add IRQ domain\n", node);
		return PTR_ERR(icu_domain);
	}
	...
	ltq_perfcount_irq = irq_create_mapping(icu_domain,
					       LTQ_ICU_IM4_PERF_IRQ);
