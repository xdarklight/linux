...
{
	struct ltq_msi_pic_data *priv = irq_data_get_irq_handler_data(data);
					   ...
					   struct msi_msg *msg)
{
	...
	search_mask = BIT(msi_desc->nvec_used) - 1;

	msg->address_hi = upper_32_bits(priv->msi_addr);
	msg->address_lo = lower_32_bits(priv->msi_addr);
	msg->data = (BIT(data->hwirq) + LANTIQ_MSI_BASE_DATA) & ~search_mask;
}
...
static int __init lantiq_msi_pic_init(struct device_node *node,
				      struct device_node *parent)
{
	...
	priv->nr_irqs = of_property_read_u32_array(node,
						   "lantiq,msi-irqs",
						   priv->irqs,
						   ARRAY_SIZE(priv->irqs));
	if (priv->nr_irqs) {
		ret = priv->nr_irqs;
		goto err_free;
	}
	...
	for (i = 0; i < priv->nr_irqs; i++) {
		...
		if ((priv->irqs[i] + 1) != priv->irqs[i + 1]) {
			pr_err("%pOF: The MSI IRQ numbering must be continuous\n",
			       node);
			ret = -EINVAL;
			goto err_free;
		}
	...
	priv->pic_base = of_iomap(node, 0);
	if (!priv->pic_base) {
		pr_err("%pOF: Unable to map the PIC registers\n", node);
		goto err_free_msi_bitmap;
	}

	ret = of_address_to_resource(node, 1, &msi_resource);
	if (!priv->pic_base) {
		pr_err("%pOF: Unable to get the MSI resource\n", node);
		goto err_unmap_pic;
	}
	...
	if (ret) {
		pr_err("%pOF: Failed to enable the MSI clock\n", node);
		goto err_put_clk;
	}
	...
	middle_domain = irq_domain_create_linear(fwnode, priv->nr_irqs,
						 ...
						 priv);
	if (!middle_domain) {
		pr_err("%pOF: Failed to create the MSI middle domain\n", node);
		ret = -ENOMEM;
	...
	msi_domain = pci_msi_create_irq_domain(fwnode,
					       ...
					       middle_domain);
	if (!msi_domain) {
		pr_err("%pOF: Failed to create MSI domain\n", node);
		ret = -ENOMEM;
...
IRQCHIP_DECLARE(alpine_msix, "lantiq,xrx200-msi-pic", lantiq_msi_pic_init);
