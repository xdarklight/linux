...
static void phy_meson_gxl_usb2_set_port_power(struct phy *phy, bool power_on)
{
	struct phy_meson_gxl_usb2_drv *drv_priv =
		dev_get_drvdata(phy->dev.parent);
	struct phy_meson_gxl_usb2_priv *phy_priv = phy_get_drvdata(phy);
	int i;
	bool enter_reset = true;

	mutex_lock(&drv_priv->reset_lock);

	phy_priv->power_on = power_on;

	/* determine if any PHY port is powered on/requesting power */
	for (i = 0; i < drv_priv->num_ports; i++) {
		phy_priv = phy_get_drvdata(drv_priv->ports[i]);

		if (phy_priv->power_on) {
			enter_reset = false;
			break;
		}
	}

	/* we must always manage all PHY ports at one. none of the PHY ports
	 * will not work as long as any port is still in reset mode. */
	for (i = 0; i < drv_priv->num_ports; i++) {
		phy_priv = phy_get_drvdata(drv_priv->ports[i]);

		regmap_update_bits(phy_priv->regmap, U2P_R0,
				   U2P_R0_POWER_ON_RESET,
				   enter_reset ? U2P_R0_POWER_ON_RESET : 0);
	}

	mutex_unlock(&drv_priv->reset_lock);
}
...
static int phy_meson_gxl_usb2_power_on(struct phy *phy)
{
	...
	phy_meson_gxl_usb2_set_port_power(phy, true);

	ret = phy_meson_gxl_usb2_set_mode(phy, priv->mode);
	if (ret) {
		phy_meson_gxl_usb2_set_port_power(phy, false);
