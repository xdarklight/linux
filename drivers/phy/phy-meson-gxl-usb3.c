...
static int phy_meson_gxl_usb3_update_mode(struct phy *phy)
{
	...
	regmap_read(priv->regmap, USB_R5, &regval);
	if (regval & USB_R5_ID_DIG_CURR) {
	...
	if (ret) {
		dev_err(&phy->dev,
			"Failed to update the USB2 PHY mode to %d\n", mode);
		return ret;
	}
...
static int phy_meson_gxl_usb3_power_on(struct phy *phy)
{
	...
	ret = phy_meson_gxl_usb3_update_mode(phy);
	if (ret)
		return ret;
...
static irqreturn_t phy_meson_gxl_usb3_otg_irq(int irq, void *data)
{
	u32 regval;
	struct phy_meson_gxl_usb3_priv *priv = data;

	regmap_read(priv->regmap, USB_R5, &regval);
	if (!(regval & USB_R5_ID_DIG_IRQ)) {
...
static int phy_meson_gxl_usb3_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(priv->usb2_phy)) {
		if (PTR_ERR(priv->usb2_phy) == -ENODEV) {
			priv->usb2_phy = NULL;
		} else {
	...
	ret = devm_request_irq(dev, irq, phy_meson_gxl_usb3_otg_irq,
			       IRQF_SHARED, dev_name(dev), priv);
	if (ret) {
		dev_err(dev, "could not register IRQ handler\n");
		return ret;
	}
