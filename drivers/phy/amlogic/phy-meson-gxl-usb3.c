...
static int phy_meson_gxl_usb3_init(struct phy *phy)
{
	...
	ret = clk_prepare_enable(priv->clk_otg);
	if (ret)
		goto err_disable_clk_phy;

	ret = phy_meson_gxl_usb3_set_mode(phy, priv->mode);
	if (ret)
		goto err_disable_clk_otg;
...
err_disable_clk_otg:
	clk_disable_unprepare(priv->clk_otg);
err_disable_clk_phy:
...
static int phy_meson_gxl_usb3_probe(struct platform_device *pdev)
{
	...
	priv->clk_otg = devm_clk_get(dev, "otg");
	if (IS_ERR(priv->clk_otg))
		return PTR_ERR(priv->clk_otg);
