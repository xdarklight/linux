...
static int phy_meson_cvbs_dac_probe(struct platform_device *pdev)
{
	...
	if (np) {
		...
		ret = phy_create_lookup(phy, "cvbs-dac", dev_name(&pdev->dev));
		if (ret)
			return dev_err_probe(dev, ret,
					     "Failed to create PHY lookup\n");
	}
...
static int phy_meson_cvbs_dac_remove(struct platform_device *pdev)
{
	struct phy *phy = platform_get_drvdata(pdev);

	if (!pdev->dev.of_node)
		phy_remove_lookup(phy, "cvbs-dac", dev_name(&pdev->dev));
...
static struct platform_driver phy_meson_cvbs_dac_driver = {
	...
	.remove = phy_meson_cvbs_dac_remove,
};
