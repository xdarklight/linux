					     ...
					     unsigned int metal_rev)
{
	...
	return kasprintf(GFP_KERNEL, "%x:%x - 0:%x", major_ver, minor_ver,
			 misc_ver);
}
...
int __init meson_mx_socinfo_init(void)
{
	...
	analog_top_regmap =
		syscon_regmap_lookup_by_compatible("amlogic,meson-analog-top");
	if (IS_ERR(analog_top_regmap))
		analog_top_regmap = NULL;
