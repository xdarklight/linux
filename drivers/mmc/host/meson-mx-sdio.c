				   ...
				   struct mmc_command *cmd)
{
	 ...
	 */
	meson_mx_mmc_soft_reset(slot->host); // TODO: do we want this?
...
{
	struct meson_mx_mmc_slot *slot = mmc_priv(mmc);
	u32 irqc = readl(slot->host->base + MESON_MX_SDIO_IRQC);
...
static irqreturn_t meson_mx_mmc_process_cmd_irq(struct meson_mx_mmc_slot *slot,
						u32 irqs, u32 send)
{
	struct mmc_command *cmd = slot->cmd;

	if (!cmd) // FIXME: WARN_ON(!cmd) causes splat during boot
		return IRQ_HANDLED;
...
static irqreturn_t meson_mx_mmc_irq_thread(int irq, void *irq_data)
{
	...
	struct mmc_command *cmd = slot->cmd, *next_cmd;
...
static void meson_mx_mmc_timeout(unsigned long arg)
{
	...
	if (!host->slot->cmd) // TODO: did we race with the IRQ thread?
		return;
