...
static void meson_mx_sdhc_wait_cmd_ready(struct mmc_host *mmc)
{
	...
	if (ret) {
		dev_warn(mmc_dev(mmc), "Failed to poll for ESTA[13:11]\n");
		meson_mx_sdhc_hw_reset(mmc);
	}
}
				    ...
				    struct mmc_command *cmd)
{
	...
	if (cmd->data) {
		...
		if (meson_mx_sdhc_cmd_needs_manual_stop(cmd))
...
static irqreturn_t meson_mx_sdhc_irq(int irq, void *data)
{
	...
	if (cmd->error || (cmd->data && cmd->data->error))
		dev_err(mmc_dev(host->mmc), "CMD%d error: %d, data error: %d, ISTA: 0x%08x\n", cmd->opcode, cmd->error, cmd->data ? cmd->data->error : 0, ista);
#endif
...
static int meson_mx_sdhc_probe(struct platform_device *pdev)
{
	...
	mmc->max_busy_timeout = 30 * 1000; // FIXME
