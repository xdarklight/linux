...
#endif
	dev_err(mmc_dev(mmc), "STAT = 0x%08x", readl(host->base + MESON_SDHC_STAT));
...
static irqreturn_t meson_mx_sdhc_irq(int irq, void *data)
{
	...
	printk("ICTL = 0x%08x, ISTA = 0x%08x\n", ictl, ista);
	...
	writel(ista, host->base + MESON_SDHC_ISTA);
...
static int meson_mx_sdhc_register_clks(struct meson_mx_sdhc_host *host)
{
	...
	mux_clk = meson_mx_sdhc_register_clk(mmc_dev(host->mmc),
					     ...
					     mux_parents, CLK_SET_RATE_NO_REPARENT | 0 /* HACK - should be 0 */, &clk_mux_ops);
	if (IS_ERR(mux_clk))
	...
	clk_set_parent(mux_clk, host->parent_clks[2].clk);
