...
static inline u8 reset_id_to_reg_bit(unsigned long id)
{
	return BIT(id & 0xff);
}

static inline int reset_reg_offset_and_bit_to_id(unsigned offset, unsigned bit)
{
	return (offset << 8) | bit;
}
				...
				unsigned long id)
{
	...
	reg = reset_id_to_reg_offset(id);

	val = ltq_rcu_r32(reg);
	val |= reset_id_to_reg_bit(id);
	ltq_rcu_w32(val, reg);
