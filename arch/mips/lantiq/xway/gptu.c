...
static struct clk_onecell_data gptu_timer_clk_data;
...
static void gptu_register_clks(struct platform_device *pdev)
{
	...
	clk_table = kcalloc(NUM_GPTU, sizeof(struct clk *), GFP_KERNEL);
	if (!clk_table) {
		panic("%s: Failed to allocate GPTU table memory\n", __func__);
		return;
	}

	clk_table[0] = gptu_register_timer_clk(&pdev->dev, "timer1a", TIMER1A);
	clk_table[1] = gptu_register_timer_clk(&pdev->dev, "timer1b", TIMER1B);
	clk_table[2] = gptu_register_timer_clk(&pdev->dev, "timer2a", TIMER2A);
	clk_table[3] = gptu_register_timer_clk(&pdev->dev, "timer2b", TIMER2B);
	clk_table[4] = gptu_register_timer_clk(&pdev->dev, "timer3a", TIMER3A);
	clk_table[5] = gptu_register_timer_clk(&pdev->dev, "timer3b", TIMER3B);

	gptu_timer_clk_data.clk_num = NUM_GPTU;
	gptu_timer_clk_data.clks = clk_table;
