...
aiu_fifo_get_component(struct snd_pcm_substream *s)
{
	struct snd_soc_pcm_runtime *rtd = s->private_data;

	return rtd->cpu_dai->component;
}

static snd_pcm_uframes_t aiu_fifo_pointer(struct snd_pcm_substream *substream)
{
	struct snd_pcm_runtime *runtime = substream->runtime;
	struct snd_soc_component *component = aiu_fifo_get_component(substream);
